class RecurringTask < ActiveRecord::Base
  ...
  # NOTE: attr_accessible was removed in Rails 4+, replaced with Strong Parameters in controller
  ...
  # Moved to helper method to avoid issues with l() method in class context
  def self.interval_units_localized
    {
      INTERVAL_DAY => I18n.t(:interval_day, :default => 'day'),
      INTERVAL_WEEK => I18n.t(:interval_week, :default => 'week'),
      INTERVAL_MONTH => I18n.t(:interval_month, :default => 'month'),
      INTERVAL_YEAR => I18n.t(:interval_year, :default => 'year')
    }
  end
  MONTH_MODIFIERS_LOCALIZED = {
    MONTH_MODIFIER_DAY_FROM_FIRST => I18n.t(:month_modifier_day_from_first, :default => 'day from first'),
    MONTH_MODIFIER_DAY_TO_LAST => I18n.t(:month_modifier_day_to_last, :default => 'day to last'),
    MONTH_MODIFIER_DOW_FROM_FIRST => I18n.t(:month_modifier_dow_from_first, :default => 'day of week from first'),
    MONTH_MODIFIER_DOW_TO_LAST => I18n.t(:month_modifier_dow_to_last, :default => 'day of week to last')
  }
  ...
  validates_inclusion_of :interval_unit,
    :in => [INTERVAL_DAY, INTERVAL_WEEK, INTERVAL_MONTH, INTERVAL_YEAR],
    :message => "Invalid interval '%{value}' (Validation)"
  validates_inclusion_of :interval_modifier,
    :in => RecurringTask::MONTH_MODIFIERS_LOCALIZED.keys,
    :message => "Invalid modifier '%{value}' (Validation)",
  ...
  def interval_localized_name
    if new_record?
      ...
      #41
      if self.class.interval_units_localized.has_key?(interval_unit)
        self.class.interval_units_localized[interval_unit]
#      case interval_unit
#      when INTERVAL_DAY
#        l(:interval_day)
#      when INTERVAL_WEEK
...
#        l(:interval_year)
        ...
        raise "Invalid interval #{interval_unit} (interval_localized_name)"
      end
    end
  end
...
##        INTERVAL_DAY
 ...
 # end
  ...
  def self.get_interval_from_localized_name(value)
    retval = self.interval_units_localized.key(value)
    if retval.nil?
      raise "Invalid interval #{value} (interval_localized_name=)"
    end
    retval
  end
  ...
  def interval_localized_modifier
    if new_record?
      ...
      if modifiers_names.has_key?(interval_modifier)
        ...
        raise "Invalid modifier #{interval_modifier} (interval_localized_modifier)"
      end
    end
  end
...
###      interval_number.days
 ...
 # end
    ...
    if prev_date.nil? 
      ...
      when INTERVAL_MONTH
        ...
        when MONTH_MODIFIER_DOW_TO_LAST
          ...
          raise "Invalid modifier #{interval_modifier} (next_scheduled_recurrence)"
        end
      when INTERVAL_YEAR
        (prev_date + interval_number.years).to_date
      else
        raise "Invalid interval #{interval_unit} (next_scheduled_recurrence)"
      end
  ...
  def recur_issue_if_needed!
    ...
    while need_to_recur?
      ...
      if Setting.plugin_recurring_tasks['journal_attributed_to_user'].blank?
        ...
        issue.init_journal(defined_user, 'Tarefa Recorrente')
  ...
  def recurrence_to_s
    ...
    schedule = fixed_schedule ? 'Cronograma Fixo' : 'Cronograma Dependente'
    "Padrão de Recorrência: #{interval_number} #{interval_localized_name.pluralize(interval_number)}#{modifier}, #{schedule}"
  end
